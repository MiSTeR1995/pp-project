...
function calc() {
  ...
  var ccalResult = document.querySelector('.calculating__result span'); // также зададим некоторые дефолтные параметры

  var sex, height, weight, age, ratio; // ratio будем получать из data аттрибутов
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(function (elem) {
      elem.classList.remove(activeClass); // id от блока с активностью сюда не попадут, так как их тупо
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // функция по подсчету, причем запускать будем каждый раз, когда пользователь
  ...
  function calcTotal() {
    ...
    if (sex === 'female') {
      ...
      ccalResult.textContent = Math.round((447.2 + 9.2 * weight + 3.1 * height - 4.3 * age) * ratio);
    } else {
      ccalResult.textContent = Math.round((88.36 + 13.4 * weight + 4.8 * height - 5.7 * age) * ratio);
    }
  ...
  calcTotal(); // Для начала получим данные со статического контента
  ...
  function getStaticInfo(selector, activeClass) {
    ...
    var elements = document.querySelectorAll(selector); // также мы будем отслеживать клики по родителю, это делегирование событий (удалено)
    ...
    elements.forEach(function (elem) {
      elem.addEventListener('click', function (e) {
        ...
        if (e.target.getAttribute('data-ratio')) {
          ...
          ratio = +e.target.getAttribute('data-ratio'); // запоминаем выбор пользователя через локальное хранилище
  ...
  getStaticInfo('.calculating__choose_big div', 'calculating__choose-item_active'); // функция по обработке инпутов (динамического контента)

  function getDynamicInfo(selector) {
    // получим тот инпут с которым будем работать
    var input = document.querySelector(selector); // обработчик события на инпут
...
function cards() {
  ...
  var MenuCard = /*#__PURE__*/function () {
    function MenuCard(title, descr, price, imgSrc, imgAlt, parentSelector) {
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        ...
        var element = document.createElement('div'); // назначим сами в rest параметр по умолчанию
        ...
        element.innerHTML = "\n                <img src=".concat(this.imgSrc, " alt=").concat(this.imgAlt, ">\n                <h3 class=\"menu__item-subtitle\">").concat(this.title, "</h3>\n                <div class=\"menu__item-descr\">").concat(this.descr, "</div>\n                <div class=\"menu__item-divider\"></div>\n                <div class=\"menu__item-price\">\n                    <div class=\"menu__item-cost\">\u0426\u0435\u043D\u0430:</div>\n                    <div class=\"menu__item-total\"><span>").concat(this.price, "</span> \u0440\u0443\u0431/\u0434\u0435\u043D\u044C</div>\n                </div>\n            ");
        this.parent.append(element);
      } // моя реализация верстки

    }, {
      key: "formMenu",
      value: function formMenu() {
        var cardNum = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
        var menuArr = [];
        var menuItem = document.querySelectorAll('.menu__item');
        menuItem.forEach(function (item, i) {
          menuArr[i] = item;
        });
        var h3 = menuArr[cardNum].querySelector('.menu__item-subtitle');
        h3.innerHTML = "".concat(this.title);
        var text = menuArr[cardNum].querySelector('.menu__item-descr');
        text.innerHTML = "".concat(this.descr);
        var price = menuArr[cardNum].querySelector('span');
        price.innerHTML = "".concat(this.price);
        var img = menuArr[cardNum].querySelector('img');
        img.src = this.imgSrc;
      }
    }]);
  ...
  (0,_services_services__WEBPACK_IMPORTED_MODULE_3__.getResource)('http://localhost:3000/menu') // придет обычный объект, не json и можем сразу его использовать
...
/* harmony default export */ __webpack_exports__["default"] = (cards); // Вариант с использовнием axios.
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault(); // создадим элемент для спиннера загрузки

      var loadingStatus = document.createElement('img');
      loadingStatus.src = message.loading;
      loadingStatus.style.cssText = "\n                display: block;\n                margin: 0 auto;\n            ";
      form.insertAdjacentElement('afterend', loadingStatus); // сбор данных с формы

      var formData = new FormData(form); // трансформируем formData в объект
      // const object = {};
      ...
      var json = JSON.stringify(Object.fromEntries(formData.entries())); // теперь этот объект можно конвертнуть в json
      ...
      (0,_services_services__WEBPACK_IMPORTED_MODULE_9__.postData)('http://localhost:3000/requests', json) // получение внятного ответа от сервера. Превратим ответ в  текст
  ...
  function postDataByXMLHTTPRequest(form) {
    form.addEventListener('submit', function (e) {
      ...
      req.open('POST', 'server.php'); // Заголовок для formData
      // req.setRequestHeader('Content-type', 'multipart/form-data');
      // для формата JSON

      req.setRequestHeader('Content-type', 'application/json'); // нам не всегда нужно передавать данные в формате json
      ...
      var formData = new FormData(form); // formData надо превратить в json, который просто так в другой формат не перегнать
      ...
      var json = JSON.stringify(object); // но сервер php не умеет нативно работать с форматом json
      ...
      req.addEventListener('load', function () {
        if (req.status === 200) {
          console.log(req.response); // оповестим пользователя о загрузке, созданным блоком

          showThanksModal(message.success);
          form.reset(); // удаление блока через 2 секунды
          // setTimeout(() => {
  ...
  function showThanksModal(message) {
    ...
    document.querySelector('.modal').append(thanksModal); // через определенное время форма должна вернуться обратно
...
function openModal(modalSelector, modalTimerId) {
  ...
  var modalWin = document.querySelector(modalSelector);
  modalWin.classList.add('show');
  modalWin.classList.remove('hide'); // блокируем прокруту страницы

  document.body.style.overflow = 'hidden'; // запускать эту очищение интервала будем в том случае, если modalTimerId
...
function closeModal(modalSelector) {
  ...
  modalWin.classList.remove('show'); // // реализация через toggle
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    ...
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight) {
      openModal(modalSelector, modalTimerId); // как только один раз польз. долистал до конца стр., то удаляем обработчик, чтобы не бесил
...
/* harmony default export */ __webpack_exports__["default"] = (modal); // создадим именованные экспорты для двух функций
...
function slider(_ref) {
  var container = _ref.container,
      ...
      currenCounter = _ref.currenCounter,
      wrapper = _ref.wrapper,
      field = _ref.field;
  // Создание слайдера
  var slides = document.querySelectorAll(slide);
  var prevSlide = document.querySelector(prevArrow);
  var nextSlide = document.querySelector(nextArrow);
  var totalSlide = document.querySelector(totalCounter);
  var currentSlide = document.querySelector(currenCounter); // переменные для карусели

  var slidesWrapper = document.querySelector(wrapper);
  var slidesField = document.querySelector(field);
  var slideW = window.getComputedStyle(slidesWrapper).width; // ширина отрендеренного блока
  // навигация по слайдам

  var slider = document.querySelector(container);
  var dots = []; // истинный массив для точек, чтобы работать потом с ними

  var offset = 0; // отступ для слайдеров

  var slideIndex = 1; // функции

  var setCurrentSlide = function setCurrentSlide(ind) {
    // После инициализации слайдера, нужно показать общее количество и текущий
    // работает для обоих вариантов, оставляем его
    if (slides.length < 10) {
      totalSlide.textContent = "0".concat(slides.length);
      currentSlide.textContent = "0".concat(ind);
    } else {
  ...
  var dotsProc = function dotsProc(dotsArray) {
  ...
  slider.style.position = 'relative'; // сначала создаем обертку для точек

  var indicators = document.createElement('ol');
  indicators.classList.add('carousel-indicators'); // можно добавить стили через cssText, но лучше напрямую в css
  ...
  for (var i = 0; i < slides.length; i++) {
    ...
    dot.classList.add('dot'); // сделаем первую точку активной изначально
  ...
  setCurrentSlide(slideIndex); // function showSlides(n) {
  ...
  slidesField.style.width = 100 * slides.length + "%"; // выстроим слайды не вертикально ,  а горизонтально в полоску
  // конечно это все можно прописать и в css, так даже будет лучше

  slidesField.style.display = 'flex';
  slidesField.style.transition = '0.5s all'; // нужно лишние элементы скрыть
  ...
  var deletNotDigits = function deletNotDigits(str) {
  ...
  nextSlide.addEventListener('click', function () {
    ...
    slidesField.style.transform = "translateX(-".concat(offset, "px)"); // контролируем текущую нумерацию
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function (e) {
      var slideTo = e.target.getAttribute('data-slide-to'); // индекс на кликнутую точку

      slideIndex = slideTo; // также нужно менять оффсет у слайдов
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  var tabsParent = document.querySelector(tabsParentSelector); // родитель для делегирования событий
  ...
  hideTabContent();
  showTabContent(); // с помощью делегирования событий сделаем обработчик

  tabsParent.addEventListener('click', function (event) {
    var target = event.target; // tabsSelector.slice(1) - так как мы в аргументе передавали селектор(точка в начале),
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endTime) {
    ...
    var t = Date.parse(endTime) - Date.parse(new Date()); // сколько осталось дней с округлением до ближайшего целого

    var days = Math.floor(t / (1000 * 60 * 60 * 24)); // Общее число часов не нужно, т.к нужны только хвостики в сутках

    var hours = Math.floor(t / (1000 * 60 * 60) % 24); // Аналогично и для минут. хвостик не более 60 минут

    var minutes = Math.floor(t / (1000 * 60) % 60); // С секундами тоже самое

    var seconds = Math.floor(t / 1000 % 60); // вернем объект оставшегося времени. Такой прием часто используется

    return {
      'total': t,
      // ост. время, нужно, чтобы знать закончился ли таймер
      'days': days,
      'hours': hours,
      'minutes': minutes,
      'seconds': seconds
  ...
  function setClock(selector, endTime) {
    ...
    var timeRemaining = getTimeRemaining(endTime); // таймер на запуск функции обновления времени каждую секунду

    var timeInterval; // let, т.к. проверям не вышел ли таймер вообще
    ...
    function updateClock() {
      ...
      seconds.innerHTML = getZero(t.seconds); // если дедлайн вышел, то останавливаем обновление таймера
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default)('.timer', '2021-02-12');
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_3__.default)();
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_4__.default)('form', modalTimerId);
  (0,_modules_calc__WEBPACK_IMPORTED_MODULE_5__.default)(); // передадим объект настроек, которые можно будет деструктуризировать
